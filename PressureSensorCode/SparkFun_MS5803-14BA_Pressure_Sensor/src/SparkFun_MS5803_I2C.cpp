...
float MS5803::getRawPressure(precision _precision)
{
	getFixed29DegreesMeasurements(_precision);
	float pressure_reported;
	pressure_reported = _pressure_actual;
	pressure_reported = pressure_reported * 10;// returns in pascals
	return pressure_reported;
}
...
void MS5803::getFixed29DegreesMeasurements(precision _precision)

{
	...
	//Now that we have a raw temperature, let's compute our actual.
	dT = temperature_raw - ((int32_t)coefficient[5] << 8);
	temp_calc = 29; //(((int64_t)dT * coefficient[6]) >> 23) + 2000;
