...
double pressure_abs, pressure_relative, pressure_raw, depth, pressure_baseline;

// Create Variable to store altitude in (m) for calculations;
double base_altitude = 329.0; // Altitude of UWaterloo in Waterloo, ON. in (m)
...
void setup() {
    ...
    //Serial.println("Baseline Reading complete");
...
void loop() {
  ...
  pressure_raw = sensor.getRawPressure(ADC_4096);
  ...
  depth = depthDelta(pressure_abs, pressure_baseline);
...
double depthDelta(double P, double P0)
{
  float rho = 995.944; //(kg/m^3) water @29 degrees C
  float g = 9.81;
  return (P-P0)/rho/g;
}
